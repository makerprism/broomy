...
export function register(ipcMain: IpcMain, ctx: HandlerContext): void {
  ...
  if (ctx.isE2ETest) {
    const sessions = getE2EDemoSessions(ctx.isScreenshotMode, ctx.e2eConversationSnapshotSeed)
  ...
  ipcMain.handle('config:load', async (_event, profileId?: string) => {
    ...
    if (ctx.isE2ETest) {
      return {
        ...
        sessions: getE2EDemoSessions(ctx.isScreenshotMode, ctx.e2eConversationSnapshotSeed),
